    ...
    QDialog(parent)
{
    ...
    hBox->addStretch();//заполнение оставшегос€ пространства
...
//генерируй по input и state таблицу заголовки выдели остальной позволь заполн€ть QTableView

    tablePV =new QTableView;
    tablePV->setFont(QFont("Arial",10,5000));
    tablePV->verticalHeader()->hide();
    tablePV->horizontalHeader()->hide();
//генераци€ модели таблицы(шапка)
    ...
    model->setItem(0, 0, item);//перва€ €чкйка
    int numState=1;
    for(int i=1; i<state*2; i+=2)//список всех состо€ний
    {
    ...
    model->setItem(1, 0, item);//перва€ €чкйка
    ...
    for(int i=1; i<=state*2; i+=2)//объединение шапки состо€ний
    {
    ...
    hBox0->addStretch();//заполнение оставшегос€ пространства
